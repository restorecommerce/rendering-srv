...
const REL_PATH_HANDLEBARS = '/handlerbars/';
const HANDLEBARS_DIR = './handlerbars';
...
describe('rendering srv testing', () => {
  ...
  before(async function start(): Promise<void> {
    ...
    console.log('customHelpersList here is ..', customHelpersList);
  ...
  //   });
    ...
    it('should render custom template 2', async () => {
      const path = cfg.get('templates:root') + cfg.get('templates:custom_helper_loud:body');
      const msgTpl = fs.readFileSync(path).toString();
      const msg = 'test';
      const renderRequest = {
        id: 'test-custom-helper',
        payload: [{
          templates: marshall({
            message: {
              body: msgTpl
      ...
      validate = () => {
        should.exist(responseID);
        should.exist(response);
        responseID.should.equal('test-custom-helper');
        console.log('response is...', response);
        console.log('response[0] is...', response[0]);

        const obj_1 = unmarshall(response[0]);
